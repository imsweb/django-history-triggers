...
class PartitionTests(TriggersTestCase):
    ...
    def test_partition_basics(self):
        with self.backend.session(username="nobody") as session:
            a = Author.objects.create(name="Nobody")
            a.name = "Somebody"
            a.save()
        ...
        update = session.history.get(change_type=TriggerType.UPDATE)
        # Check insert history.
        self.assertIs(insert.__class__, UnmanagedHistory)
        self.assertEqual(insert.session_id, session.session_id)
        self.assertEqual(insert.get_user(), "nobody")
        self.assertEqual(insert.snapshot, {"id": a.pk, "name": "Nobody"})
        self.assertIsNone(insert.changes)
        # Check update history.
        self.assertEqual(update.session_id, session.session_id)
        self.assertEqual(update.get_user(), "nobody")
        self.assertEqual(update.snapshot, {"id": a.pk, "name": "Somebody"})
        self.assertEqual(update.changes, {"name": ["Nobody", "Somebody"]})
        with connection.cursor() as c:
            # Querying the base table should automatically include partitions.
            c.execute("SELECT count(*) FROM {}".format(UnmanagedHistory._meta.db_table))
            count = c.fetchone()[0]
            self.assertEqual(count, 2)
            # Make sure the history got routed to the partition table.
            c.execute("SELECT count(*) FROM {}".format(self.partition_name))
            count = c.fetchone()[0]
            self.assertEqual(count, 2)
