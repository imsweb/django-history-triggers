...
IGNORED_PREFIXES = getattr(settings, 'HISTORY_IGNORED_PREFIXES', ('lkup_', 'django_', 'djapian_', 'auth_', 'south_'))

# Columns which should not be tracked in history tables.
IGNORED_TYPES = getattr(settings, 'HISTORY_IGNORED_TYPES', ('bytea', 'text', 'xml'))
IGNORED_COLUMNS = getattr(settings, 'HISTORY_IGNORED_COLUMNS', ('date_modified', 'date_last_modified'))
    ...
    def handle(self, *args, **options):
        try:
            ...
            for table_name in sorted(table_names):
                pk_name = table_names[table_name]
                if not dropping and create_history_table(cursor, table_name, pk_name):
                    print 'Created history table for "%s" (%s)' % (table_name, pk_name)
            ...
            raise CommandError('Failed to build history triggers: %s' % str(ex))
        else:
...
def get_base_tables(cursor, schema_name='public'):
    ...
    """
    cursor.execute("""
        SELECT t.table_name, COALESCE(kcu.column_name, '') FROM information_schema.tables t
        LEFT OUTER JOIN information_schema.table_constraints tc ON tc.table_schema = t.table_schema AND tc.table_name = t.table_name AND tc.constraint_type = 'PRIMARY KEY'
        LEFT OUTER JOIN information_schema.key_column_usage kcu ON kcu.table_schema = tc.table_schema AND kcu.table_name = tc.table_name AND kcu.constraint_name = tc.constraint_name
...
def create_history_table(cursor, base_table, pk_name):
    """
...
def get_field_history_sql(trigger_type, table_name, field_name, pk_name):
    params = {
        'field': field_name,
...
def create_trigger(cursor, trigger_type, table_name, pk_name, table_schema='public'):
    ...
    body_sql = []
    for field_name in get_table_columns(cursor, table_name, table_schema):
        sql = get_field_history_sql(trigger_type, table_name, field_name, pk_name)
        body_sql.append(sql)
